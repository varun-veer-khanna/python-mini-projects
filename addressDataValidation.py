#Python program to input data into SQL database using Tkinter GUI with data validation.
from tkinter import *
...
class Contact:
    ...
    #def onClick():
        #tkinter.messagebox.showinfo("Alert",  "Invalid input")    
    def add(self, *args):
        try:
            
            conname=self.name.get()
            connumber=self.number.get()
            if not connumber.isdigit() or not(len(connumber)==10):
                #self.onClick
                tkinter.messagebox.showinfo("Alert",  "Invalid input")
            def check(email): 
            ...
            #conmail2=check(conmail)
            if not check(conmail):
                tkinter.messagebox.showinfo("Alert",  "Invalid input")
                #self.onClick()
